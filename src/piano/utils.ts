...
export const KEYS: PianoKey[] = [
  { note: "C3", freq: 130.81, type: "white", label: "Q" },
  { note: "C#3", freq: 138.59, type: "black", label: "2" },
  { note: "D3", freq: 146.83, type: "white", label: "W" },
  { note: "D#3", freq: 155.56, type: "black", label: "3" },
  { note: "E3", freq: 164.81, type: "white", label: "E" },
  { note: "F3", freq: 174.61, type: "white", label: "R" },
  { note: "F#3", freq: 185.0, type: "black", label: "5" },
  { note: "G3", freq: 196.0, type: "white", label: "T" },
  { note: "G#3", freq: 207.65, type: "black", label: "6" },
  { note: "A3", freq: 220.0, type: "white", label: "Y" },
  { note: "A#3", freq: 233.08, type: "black", label: "7" },
  { note: "B3", freq: 246.94, type: "white", label: "U" },

  { note: "C4", freq: 261.63, type: "white", label: "Z" },
  { note: "C#4", freq: 277.18, type: "black", label: "S" },
  ...
  { note: "F#4", freq: 369.99, type: "black", label: "G" },
  { note: "G4", freq: 392.0, type: "white", label: "B" },
  { note: "G#4", freq: 415.3, type: "black", label: "H" },
  { note: "A4", freq: 440.0, type: "white", label: "N" },
  { note: "A#4", freq: 466.16, type: "black", label: "J" },
...
export const startPlayback = (
  ...
  onPlayNote: (ev: AudioEngineEvent, when: number, duration: number) => void,
  onStopNote: (ev: AudioEngineEvent) => void,
  ctx: AudioContext,
  ...
  const fallbackLength =
    events.length > 0 ? (events[events.length - 1].end ?? 1) : 1;
  const loopLen = loopLength > 0 ? loopLength : fallbackLength;
  const isLooping = loopLength !== -1;

  const windowStart = elapsed;
  const windowEnd = elapsed + scheduleAheadTime;

  const currentCycle = isLooping ? Math.floor(elapsed / loopLen) : 0;
  events.forEach((ev, index) => {
    const cyclesToCheck = isLooping ? [currentCycle, currentCycle + 1] : [0];

    for (const cycle of cyclesToCheck) {
      const cycleStart = cycle * loopLen;
      const noteStartInTimeline = cycleStart + ev.start;

      if (
        noteStartInTimeline >= windowStart &&
        noteStartInTimeline < windowEnd
      ) {
        //
        const eventKey = `${index}_${ev.note}_cycle${cycle}_start${ev.start}`;
        if (!scheduledNotes.has(eventKey)) {
          scheduledNotes.add(eventKey);

          const when = playStartTime + noteStartInTimeline;
          const duration = (ev.end ?? ev.start + 0.2) - ev.start;

          onPlayNote(ev, when, duration);

          setTimeout(
            () => {
              scheduledNotes.delete(eventKey);
              onStopNote(ev);
            },
            (scheduleAheadTime + duration) * 1000,
          );
        }
      }
    }
